...
# Сколько секунд судья НЕ детектит аут/лицевую сразу после того, как любой сет-пис-контроллер
# отпустил мяч. Нужно только вбросу (единственный стандарт, где точка розыгрыша ЛЕЖИТ на самой
# границе — мяч физически стоит за линией в руках вбрасывающего): без грейса судья видит мяч ещё
# не отъехавшим за порог (|x| > half_width + ball_radius) в тот же/следующий кадр, что и снятие
# _throw_in_active, и тут же переучреждает НОВЫЙ вброс с нулевой скоростью — мяч выглядит так,
# будто врезался в невидимую стену на боковой и достаётся вбрасывающему обратно. Остальные
# стандарты кладут мяч заметно внутри поля и грейс им не нужен, но проверка общая для всех пяти —
# безопаснее и не требует различать источник.
const REFEREE_SETPIECE_GRACE := 0.5
