...
func _check_runup() -> bool:
	var runup := 2.8
	# ЛЕВЫЙ угол глазами вратаря (side=+1, into=1): x=33.5, z=-52.0. Боковая x=34, линия ворот z=-52.5.
	# Тут ПРАВАЯ нога — из-за БОКОВОЙ (x>34), ЛЕВАЯ — из-за ЛИНИИ ВОРОТ (z<-52.5).
	var spot := CornerLogic.corner_spot(1.0, 34.0, -52.5, 0.5, 0.11)
	...
	if not (sr.x > 34.0 and sr.z > -52.5):
		print("  FAIL runup R region side+ (ожидалось x>34, из-за боковой): ", sr)
		return false
	if sl.z >= -52.5:
		print("  FAIL runup L region side+ (ожидалось z<-52.5, из-за линии ворот): ", sl)
		return false
	# ПРАВЫЙ угол (side=-1) — ЗЕРКАЛО: правая нога из-за ЛИНИИ ВОРОТ (z<-52.5), левая из-за БОКОВОЙ (x<-34).
	...
	if sr2.z >= -52.5:
		print("  FAIL runup R region side- (ожидалось z<-52.5, из-за линии ворот): ", sr2)
		return false
	if not (sl2.x < -34.0 and sl2.z > -52.5):
		print("  FAIL runup L region side- (ожидалось x<-34, из-за боковой): ", sl2)
		return false
