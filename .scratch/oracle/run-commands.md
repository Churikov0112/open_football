...
Дифф трасс — первый разошедшийся тик по дискретным полям и тик начала роста по непрерывным. Ту же
проверку манифестов он делает своей первой стадией, логика общая и живёт в `tools/oracle_manifest.gd`:
...
Аргументы после `--`: трасса эталона, трасса порта, манифест эталона, манифест порта, белый список и
**необязательный** шестой — файл допусков. Без него действуют дефолты из инструмента (см. [[константы]]).
Формат файла — строка `<ключ> <число>`, ключи `position`, `angle`, `velocity`, `smuggle`, `window`,
`#` — комментарий, неизвестный ключ валит парсер с указанием строки:

```
# поднять допуск позиции и укоротить окно правила роста
position 1e-2
window 6
```
...
Оба слоя (дискретный и непрерывный) живут в `tools/oracle_trace.gd` и покрыты этим тестом.
