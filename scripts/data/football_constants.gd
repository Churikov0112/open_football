...
# Дриблинг «толкни и догони» (аркадная петля): СТИК задаёт направление ТОЛЧКА мяча (мяч
# отскакивает туда, куда стик), а БЕГ игрока авто-направляется к мячу (match_manager) — толкнул
# и сам побежал догонять. Мяч катится своей инерцией с трением; когда игрок догнал его (в
# пределах DRIBBLE_CATCH_DIST) — новый толчок в текущем направлении стика. Толчок на «фронте»
# (мяч отошёл → догнал → толкнул), а не каждый кадр — поэтому мяч не «прилеплен» к ноге.
const DRIBBLE_CATCH_DIST := 0.5   # догнал мяч ближе этого (м) → толчок + флаг chasing=false (ведём стиком)
const DRIBBLE_CHASE_DIST := 0.9   # мяч дальше этого (м) → флаг chasing=true (игрок бежит к мячу); гистерезис с CATCH
const DRIBBLE_PUSH_BONUS := 3.5   # мяч толкается на (скорость_игрока + это), м/с — сильнее/дальше
const DRIBBLE_ROLL_DRAG := 0.97   # трение качения между толчками (выше = катится дальше)
const DRIBBLE_SPRINT_PUSH_EXTRA := 4.0  # доп. сила толчка при полном спринте (мяч дальше → сложнее контроль)
# Владение НЕ теряется от собственного толчка/резкого поворота — только по внешнему фактору
# (подкат/перехват зовут release_dribble отдельно). Флаг chasing + авто-догон возвращают мяч;
# дистанционный сброс — лишь страховка для реально улетевшего мяча (выше любой ролл-дистанции).
...
const PICKUP_ASSIST_STRENGTH := 0.6  # макс. доля подруливания к мячу (у самого мяча; линейно спадает к радиусу)
