...
**Interfaces:**
- Consumes: `KickerIntent`, `HumanKickerIntent`, `SetPiecePresentation`, `KickoffLogic.clamp_to_own_half`, `FreeKickLogic.push_out_of_radius`/`rotate_heading`, `PassSystem.ground_pass_speed`/`launch_ground`.
...
func _setup() -> void:
	...
	_base_heading = Vector3(0.0, 0.0, -_attack_sign)   # к своей половине
...
## половину (base_heading) — правило клэмпа/круга на него НЕ распространяется.
func _place_kicker() -> void:
	_kicker.global_position = _spot + Vector3(0.0, 0.0, _attack_sign) * FootballConstants.KICKOFF_KICKER_OFFSET \
...
func _dispatch_kickoff(kicking_team: int) -> void:
	...
	if kicking_team == 1:
		...
		# Те же формулы, что применит KickoffController._setup() (детерминированы, см.
		# KickoffLogic) — ИИ должен знать финальную геометрию ДО того, как контроллер её
		# выставит (тот же приём, что free_kick_controller._clear_ball_to_wall_cone
		# переиспользует формулы _spawn_defense).
		var kicker_pos := Vector3(0.0, 0.5, team.attack_z_sign * FootballConstants.KICKOFF_KICKER_OFFSET)
		var base_heading := Vector3(0.0, 0.0, -team.attack_z_sign)
