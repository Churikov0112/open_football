...
**One brain — `scripts/match/match_manager.gd`** (extends Node3D, root of `match.tscn`). This ~930-line file owns nearly everything: input setup, camera tracking, procedural field markings/goals/grass/boundaries, controlled-player switching, kick/pass, and the entire slide-tackle state machine. When in doubt, the logic is here.
...
**Constants:** `scripts/data/football_constants.gd` is autoloaded as `FootballConstants` (global). Central place for field dimensions, ball/player/AI/tackle/locomotion (`LOCO_*`) tuning. **Caveat:** `match_manager.gd` also hardcodes several values inline (kick power `18.0`, pass power `12.0`, kick range `2.0`) that duplicate or diverge from the constants — when tuning, grep both the constant and the literal. (Player top speed is no longer one of these hardcodes — it now comes from `FootballConstants.LOCO_TOP_SPEED` via `PlayerMotor`.)
