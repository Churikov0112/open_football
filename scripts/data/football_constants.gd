...
# ВАЖНО: все множители затухания ниже (`MAGNUS_DECAY`, `DRIBBLE_ROLL_DRAG`, `DRIBBLE_CONTROL_LERP`,
# `KEEPER_DIVE_DECAY`, а также `drag_factor`/`air_resistance` мяча) применяются РАЗ ЗА ФИЗКАДР и
# затюнены при 60 Гц, тогда как матч живёт на фикс-тике 100 Гц. Хранятся они в 60-герцевом виде, а к
# реальному шагу приводятся через `TickScale` (`scripts/data/tick_scale.gd`) — новое такое число
# обязано идти тем же путём, иначе смена тика молча меняет физику.
...
# Подтверждение гола (Law 10): гол — только когда мяч ВЕСЬ пересёк линию (центр за линией на
# BALL_RADIUS → задняя кромка тоже за линией). Голевая зона Area3D стоит фронтом на линии и лишь
# инициирует проверку (её body_entered срабатывает, когда передняя кромка мяча коснулась линии,
# т.е. центр ещё ПЕРЕД линией на радиус). Опрашиваем каждый физкадр до разрешения: гол — когда
# весь мяч за линией; отмена — если мяч пойман/отбит обратно в поле (центр отошёл в поле за
# GOAL_CONFIRM_RETREAT от точки входа) или истёк GOAL_CONFIRM_MAX_SEC (мяч застрял на линии — не гол).
const GOAL_CONFIRM_RETREAT := 0.15    # на сколько центр мяча должен отойти в поле ЗА точку входа, чтобы счесть отбитым
const GOAL_CONFIRM_MAX_SEC := 3.0     # предохранитель: макс. время опроса, с (в кадры переводится по фикс-тику)
...
const BALL_BOUNCE := 0.6          # доля вертикальной скорости, возвращаемой при отскоке от газона (restitution)
const BALL_BOUNCE_MIN_SPEED := 1.0 # медленнее этой скорости снижения (м/с) мяч НЕ прыгает — ложится на газон (без дрожи)
const BALL_BOUNCE_FRICTION := 0.9 # доля ГОРИЗОНТАЛЬНОЙ скорости, сохраняемой при каждом отскоке (мяч катится дальше с затуханием)
const BALL_GROUND_EPS := 0.03     # допуск высоты (м) над y=BALL_RADIUS, при котором мяч считается на газоне
...
const SETPIECE_BLOCK_LAYER := 16     # bit5: невидимая стена штрафной на стандарте соперника — её
									 # слушают ТОЛЬКО защитники (временный бит в их mask), мяч/бьющие проходят
...
const DEBUG_DISABLE_OPPONENT := false # ВРЕМЕННО: true — ИИ-соперник просто стоит + стенки болванок (тест дриблинга/ударов). false — чтобы конвертированная стенка штрафного стала активным ИИ.
...
const DEBUG_DISABLE_TEAMMATE := false # ВРЕМЕННО: true — ИИ-тиммейт не спавнится (один-в-один); false — тиммейт в team_1 (тест пасов)
const DEBUG_MARK_DISTANCE := 2.5      # дистанция маркировки: соперник встаёт между мячом и тиммейтом на столько метров
const DEBUG_BALL_TRAIL := false       # цветной след за мячом (виден путь удара) — для тестов траекторий
...
# соперника (simple_ai.gd @export var speed, на момент тюнинга 6.5, сейчас 8.0 — константа
# AI_SPEED=5.0 выше по файлу им не используется, легаси) — подкат сзади (сближение вдогонку)
# физически никогда не догонял цель, хотя сбоку (сближение поперёк) работал. SPEED поднят
# с запасом выше скорости бега ИИ, чтобы вдогонку соперника хватало (после подъёма speed
# до 8.0 запас всего 1.0 м/с — если подкат вдогонку снова перестанет догонять, причина тут). RECOVERY_TIME подобран так, чтобы суммарный commit
...
# DEBUG-переключение: кнопка 1 = broadcast (по умолчанию), 3 = этот вид — см. match_manager._process().
...
# Обычная игра (не пенальти/штрафной): ТВ-трансляция — фикс. позиция сбоку и сверху поля,
# плавно панорамирует за мячом (не за игроком). Пенальти/штрафной — отдельная камера от 3-го
# лица за бьющим (PEN_CAM_*/FK_CAM_*), не трогаются этими константами.
const CAMERA_BC_X := -40.0         # фиксированная боковая позиция камеры (X сбоку поля), м
const CAMERA_BC_HEIGHT := 18.0     # высота камеры над газоном, м
const CAMERA_BC_LOOK_Y := 1.0      # высота точки взгляда (примерно уровень мяча), м
const CAMERA_BC_FOLLOW := 3.0      # темп сглаживания панорамы за мячом вдоль поля (Z), 1/с
...
const KEEPER_LINE_NARROW_GAIN := 0.08  # на сколько метров выхода с линии теряется за 1 м удаления мяча (legacy line_position)
const KEEPER_MAX_OFF_LINE := 5.0       # макс. выход из ворот в поле на угрозу (угловое позиционирование), м
const KEEPER_BASE_OFF_LINE := 2.75     # покойный выход = центр вратарской площади (GOAL_AREA_DEPTH*0.5), м
const KEEPER_POS_LOOK := 0.12          # упреждение по скорости мяча при позиционировании, с
const KEEPER_BACKPASS_SPEED_CAP := 16.0 # мяч своих медленнее этого = пас/скидка (берём в ноги); быстрее — возможный рикошет удара (сейв руками/нырок)
const KEEPER_BACKPASS_COLLECT_RANGE := 22.0 # с какой близости вратарь-ИИ бежит собирать бэк-пас (дальше — стоит, не выбегает), м
# Бэк-пас по высоте: низкий берётся в ноги, высокий навес перелетает над головой (не «прилипает»),
# и вратарь бежит за ним, добирая опускающимся.
const KEEPER_BACKPASS_TRAP_HEIGHT := 1.3   # бэк-пас ниже этой высоты (м) вратарь принимает в ноги (в НОГИ, не руками); выше — не хватает, перелетает
...
const KEEPER_SLIDE_MAX_LATERAL := 1.5  # вбок дальше этого (м) — НЫРЯЕМ, а не выходим шагом по линии
									   # (зрелищные прыжки в углы; дайв-рэндж 5.5 покрывает весь створ).
									   # Меньше = ныряет чаще/раньше.
...
# Черпачок. Сила задаёт ДАЛЬНОСТЬ приземления (как полевой chip): мало → недолёт (перед линией 11м,
# в руки), в меру → падает в ворота, много → перелёт (мяч на линии выше перекладины → промах). Так
# полной силой можно промахнуться. Высота дуги тоже растёт с силой.
const PEN_CHIP_PEAK_MIN := 2.0         # высота дуги черпачка при мин. заряде, м
const PEN_CHIP_PEAK_MAX := 6.0         # при полном заряде, м
const PEN_CHIP_DIST_MIN := 8.0         # дальность приземления при мин. заряде, м (короче линии 11м → недолёт)
const PEN_CHIP_DIST_MAX := 18.0        # при полном заряде, м (далеко за ворота → над перекладиной, промах)
...
# Прицел/камера.
const FK_AIM_ARC := 3.15              # ±сектор поворота heading (рад ≈ 180° = полный оборот: для паса можно смотреть налево/направо/назад)
...
const FK_POWER_MIN_SPEED := 36.0      # скорость мяча при мин. заряде, м/с (даже низкий удар должен долетать)
const FK_POWER_MAX_SPEED := 46.0      # при полном заряде, м/с
...
const FK_GOAL_MAGNET := 0.18          # магнетизм: доля подмешивания направления на центр ворот к удару (0..1)
const FK_CURL_SCALE := 7.0            # накопленный боковой ввод стика → величина curl.z (единицы как CURL_STRENGTH_*)
const FK_CURL_MAX := 3.5              # кламп |curl.z| — близко к штатному CURL_STRENGTH_MAX=2.3, но чуть выразительнее
                                       # для штрафных. НЕ поднимать сильно выше: большой curl.z даёт Magnus-силу,
                                       # приложенную каждый кадр перпендикулярно ТЕКУЩЕЙ скорости — при избыточной
                                       # величине это закручивает мяч почти по кругу («кружится на месте») вместо
                                       # плавной дуги, а резкая смена направления vel.z может увести
                                       # KeeperLogic.shot_intercept в фолбэк «летит от линии» (возврат позиции
                                       # мяча вместо точки на линии ворот) → вратарь ошибочно бежит/ныряет к
                                       # текущей позиции мяча в поле, а не к воротам.
# Наземный пас: скорость из ДИСТАНЦИИ до получателя (PassSystem.ground_pass_speed, PASS_GROUND_*),
# чтобы всегда доходил — фиксированные FK_PASS_*_SPEED убраны.
...
# Расчистка конуса удара (от мяча к крайним игрокам стенки).
const FK_CONE_MARGIN_DEG := 4.0       # запас угла сектора наружу, чтобы вытолкнутые стояли явно вне конуса, град
# Вратарь.
...
const CORNER_RUNUP_ANGLE := 30.0      # отклонение разбега, град: левая заходит из-за линии ворот, правая — из-за боковой (диагональ)
# Прицел/камера.
...
# Наземный пас A: скорость выводится из ДИСТАНЦИИ до получателя (PassSystem.ground_pass_speed,
# константы PASS_GROUND_*), чтобы всегда доходил — фиксированные CORNER_PASS_*_SPEED убраны.
...
# ИИ-бьющий угловой (Этап 2). Скорер lob-first: навес в штрафную на реального тиммейта.
const AI_CORNER_THINK_TIME := 0.7     # с — пауза обдумывания ИИ перед доворотом (как AI_GOALKICK_THINK_TIME)
const AI_CORNER_UPFIELD_WEIGHT := 1.0 # вес продвижения к воротам в CornerPlan
const AI_CORNER_OPP_SPEED := 8.0      # = LOCO_TOP_SPEED, скорость соперника для оценки перехвата наземного паса
const AI_CORNER_BALL_SPEED := 12.0    # представительная скорость наземного паса для interception_time, м/с
const AI_CORNER_LOB_BIAS := 6.0       # запас дальности (м): навес предпочитается, если цель в пределах lob-диапазона + bias
# Живая защита при ИИ-угловом: соперники (защитники) не ближе этого к точке углового (Law 17, 9.15 м).
const CORNER_DEFEND_DIST := 9.15      # радиус невидимой стены вокруг углового флажка при ИИ-подаче, м
...
const GK_CLEAR_MARGIN := 5.0          # запас вокруг вратарской (5.5x9.16), где не должно быть НИКОГО
