...
const FK_CAM_SIDE_X := -40.0          # ТВ-камера: фиксированная боковая позиция (X сбоку поля), м
const FK_CAM_HEIGHT := 18.0           # высота камеры над газоном (эффект «трибунной» съёмки сверху), м
const FK_CAM_LOOK_Y := 1.0            # высота точки взгляда (примерно уровень мяча/торса), м
const FK_CAM_FOLLOW := 2.0            # темп сглаживания панорамы за мячом (плавно, не резко), 1/с
# Сила/траектория.
