...
## Penalty (Phase A)
- Press **P** to stage a penalty into the attacked goal. `scripts/match/penalty_controller.gd` (base, one penalty, `SETUP→AIM→STRIKE`, emits `struck`, `release_after_strike`) + pure math in `scripts/match/penalty_logic.gd` (`class_name PenaltyLogic`, never reads `FootballConstants`, headless-tested). Yellow world-space reticle on the goal plane; hold **kick** to charge power (ring = spread grows with power); `combo_modifier`+kick = chip. Ball = `aim + uniform sample in spread disc` (can miss). Direct+chip only.
