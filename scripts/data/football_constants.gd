...
const CAMERA_TP_DISTANCE := 7.0    # отступ камеры назад от игрока, м (не используется — заменено на broadcast-вид ниже)
const CAMERA_TP_HEIGHT := 3.5      # высота камеры над игроком, м (не используется)
const CAMERA_TP_LOOK_AHEAD := 3.0  # насколько вперёд игрока смотрит камера, м (не используется)
const CAMERA_TP_FOLLOW := 6.0      # темп сглаживания следования, 1/с (не используется)
