...
## КАКАЯ нога из какой зоны — зависит от ноги И СТОРОНЫ угла (углы зеркальны!). Для правого угла
## глазами вратаря (side>0): правая нога — из-за линии ворот, левая — из-за боковой. Для левого
## (side<0) — наоборот. Кодируется через zone_touch = (нога правая) == (сторона левая).
