...
# GOAL_CONFIRM_RETREAT от точки входа) или истёк GOAL_CONFIRM_MAX_FRAMES (мяч застрял на линии — не гол).
const GOAL_CONFIRM_RETREAT := 0.15    # на сколько центр мяча должен отойти в поле ЗА точку входа, чтобы счесть отбитым
const GOAL_CONFIRM_MAX_FRAMES := 180  # предохранитель: макс. физкадров опроса (~3с), потом — не гол
