...
## СНАРУЖИ поля у углового флажка (как в реальном угловом) и заходит к мячу с двух разных зон по
## ноге (бьющий стоит на runup_dir*dist позади мяча = снаружи):
##   • левая нога — из-за ЛИНИИ ВОРОТ: заход преимущественно в поле (+into по Z), слегка к центру;
##   • правая нога — из-за БОКОВОЙ линии, перед линией ворот: заход по диагонали к центру (-side по
##     X) со сдвигом к лицевой линии (-into по Z).
## angle_deg — доля отклонения: у левой это увод в поле по X, у правой — доворот к линии ворот по Z.
