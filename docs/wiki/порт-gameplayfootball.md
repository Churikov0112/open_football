...
## Статус: фаза 3 (`CalculatePhysicsVector`) код-complete, визуальная приёмка впереди

Фаза 1 («лаб-сцена + скелет-палочник») и фаза 2 (`AnimCollection`, выбор клипа) закрыты и приняты
визуально человеком. Фаза 3 (варпинг траектории под физику: `SpatialState`, `PhysicsVector.Calculate`
== `CalculatePhysicsVector`, контур состояния `HumanoidBase` — тик, выбор следующего движения-клипа,
apply-буфер; `walk_lab` переведён с lite-дескрипторов фазы 2 на настоящую интеграцию) реализована и
код-complete: все 17 headless-тестов `tests/check_gpf_*.gd` зелёные, обе стандартные headless-валидации
без новых категорий ошибок, `dotnet build` чист. **Интерактивная визуальная приёмка человеком**
(палочник в `walk_lab.tscn` разгоняется/тормозит/поворачивает непрерывно, без квантованных скачков по
бакетам скорости) и финальное ревью ветки — ещё не пройдены, идут следом за этой документационной
задачей. Следующая веха после приёмки — фаза 4 (smuggle + физика мяча).
