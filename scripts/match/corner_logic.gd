...
## Горизонтальное направление разбега бьющего к мячу (куда он БЕЖИТ и лицом). Разбег стартует
## СНАРУЖИ поля у флажка (как в реальном угловом) и заходит к мячу с одной из двух зон:
##   • «из-за БОКОВОЙ линии», перед линией ворот — заход по диагонали к центру (-side по X) со
##     сдвигом к лицевой (-into по Z);
##   • «из-за ЛИНИИ ВОРОТ» — заход в поле (+into по Z), слегка к центру по X.
## КАКАЯ нога из какой зоны — зависит от ноги И СТОРОНЫ угла (углы зеркальны!). Правый угол глазами
## вратаря = side<0: там правая нога заходит из-за ЛИНИИ ВОРОТ, левая — из-за БОКОВОЙ. Левый угол
## (side>0) — наоборот (правая из-за боковой). Кодируется zone_touch = (нога правая) == (сторона>0).
## (Если в игре стороны окажутся перепутаны — поменять `>` на `<` в сравнении side.)
## angle_deg — доля отклонения захода.
static func runup_dir(side: float, into: float, foot: String, angle_deg: float) -> Vector3:
	var a := deg_to_rad(angle_deg)
	var zone_touch := (foot == "penalty_r") == (side > 0.0)
	if zone_touch:
		# Из-за БОКОВОЙ линии, перед линией ворот.
		return Vector3(-side * cos(a), 0.0, -into * sin(a)).normalized()
	# Из-за ЛИНИИ ВОРОТ, заход в поле.
	return Vector3(-side * sin(a), 0.0, into * cos(a)).normalized()
