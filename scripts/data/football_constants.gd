...
const SHOT_POWER_MIN := 22.0        # горизонтальная скорость при мин. заряде, м/с
...
const CHIP_DIST_MIN := 12.0         # дальность приземления при мин. заряде, м (слабый — близко роняет)
