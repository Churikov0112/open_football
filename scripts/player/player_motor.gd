...
func _physics_process(delta: float) -> void:
	...
	# в подкате, либо автомат падения _process_fall при fallen — двигает global_position
	# напрямую), motor вертикаль не трогает — иначе move_and_slide() ниже спорит за Y с той
	# системой в тот же кадр (тот же риск, что и комментарий про гашение скорости выше,
	# только по вертикали).
	if locked_or_fallen:
		_vy = 0.0
		new_vel.y = 0.0
