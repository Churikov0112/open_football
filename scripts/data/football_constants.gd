...
const PASS_GROUND_MAX_TRAVEL_TIME := 0.9   # время полёта при минимальном заряде (мягкий, медленный пас), с
const PASS_GROUND_MIN_SPEED := 6.0         # нижний предел скорости мяча низом, м/с
...
const PASS_RECEIVE_PREDICT_WINDOW := 0.04  # на сколько сек вперёд предсказываем позицию мяча
