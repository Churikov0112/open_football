...
## Явный сброс кручения (Magnus) + визуального вращения. Зовётся при ЛЮБОМ контакте мяча с
## игроком: траектория уже нарушена, дальнейшее подкручивание/вращение выглядит противоестественно.
## Отдельно от block_in_flight, т.к. тот срабатывает лишь пока мяч в FLIGHT; при контакте с уже
## выпавшим в OPEN мячом (медленным) кручёный _curl/angular иначе оставались бы ненулевыми.
func clear_curl() -> void:
	_curl = Vector3.ZERO
	angular_velocity = Vector3.ZERO
