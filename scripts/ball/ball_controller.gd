...
## Явный сброс кручения (Magnus). Зовётся при ЛЮБОМ контакте мяча с игроком: траектория уже
## нарушена, дальнейшее подкручивание выглядит противоестественно («мяч кружит на месте»).
## Отдельно от block_in_flight, т.к. Magnus в _integrate_forces работает и в OPEN (гейт только
## «в воздухе + есть скорость»), а block_in_flight вызывается лишь пока мяч в FLIGHT.
