...
IN_PLACE_CLIPS = {
    ...
    "roll_left": (0, 1, 2),
    "roll_right": (0, 1, 2),
    # Прыжок стенки при штрафном: морозим ВСЁ (0,1,2) — вертикаль прыжка ведёт контроллер вручную
    # (поднимает всё тело CharacterBody), клип даёт только позу «прыжка вверх» в костях; иначе
    # вертикаль клипа сложилась бы с ручным подъёмом (двойной поп).
    "jumping_wall": (0, 1, 2),
    # --- Вратарь ---
    ...
    # Верховые нырки: морозим ТОЛЬКО горизонталь (0,2), вертикаль (1) ЖИВАЯ — клип сам делает
    # прыжок вверх и опускание на газон. Заморозка вертикали (0,1,2) оставляла таз на высоте
    # стойки → модель «висела в воздухе» в позе нырка. Физике отдаём только горизонталь.
    "keeper_diving_save_l": (0, 2),
    "keeper_diving_save_r": (0, 2),
    "keeper_catch": (0, 2),
    "keeper_catch_top": (0, 2),
    "keeper_catch_head": (0, 2),  # сейв на уровне головы — ловля на месте, горизонталь морозим
    "keeper_scoop": (0, 2),       # подбор низкого мяча на месте, вертикаль (наклон) живая
    "keeper_miss_top": (0, 2),    # прыжок вверх мимо: вертикаль (прыжок) живая
    "keeper_sidestep": (0, 2),
    # Вынос ногой на месте: морозим горизонталь (0,2), иначе root motion таскает модель (и мяч
    # в руке) вперёд во время клипа. Вратарь при выносе стоит (мотор тоже заблокирован).
