...
# Верховой на ход — ПОЛНОСТЬЮ РУЧНОЙ: направление строго по стику, дальность/сила по ЗАРЯДУ, без
# автонаводки/разброса/хендоффа. Дальность = стик * lerp(MIN,MAX,заряд): тап ≈ минимум, полный
# заряд ≈ пол поля (HALF_FIELD_LENGTH = 52.5 м). Тиммейт сам реагирует ИИ (бежит за мячом).
const PASS_THROUGH_AIR_SPACE_MIN := 10.0   # дальность по стику при тапе, м (твёрдый минимум)
const PASS_THROUGH_AIR_SPACE_MAX := 52.0   # при полном заряде — ~пол поля
