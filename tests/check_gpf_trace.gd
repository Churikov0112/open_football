...
#
# Мяч стоит ДАЛЬШЕ 16 м от игрока намеренно. Порог 16 м (`humanoid.cpp:1565-1566`) переключает
# desiredIdleLevel, а с ним и idle-клип, на котором стоит игрок; правило нулевого тика сверяет
# текущий клип с GetIdleMovementAnimID(), и внутри 16 м эти два отбора дают РАЗНЫЕ клипы — условие
