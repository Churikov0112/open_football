...
# PEN_RUNUP_DIST = накопленный разбег таза К МОМЕНТУ PEN_CONTACT_TIME (двигаются вместе: пред-
# расстановка на эту дистанцию назад → тело синхронно с анимацией к кадру удара). Контакт чуть
# раньше конца разбега — бьющая нога достаёт мяч, пока таз ещё чуть позади.
const PEN_RUNUP_DIST := 2.6           # разбег таза к контакту (мировые метры), кладём бьющего на неё назад
const PEN_ROOT_SCALE := 0.0367        # raw get_root_motion_position → игровые метры (Hips-трек в сырых единицах)
const PEN_CONTACT_TIME := 1.15         # кадр контакта клипа penalty_kick_* (сек), клип ~1.53с
...
const PEN_FOOT_LATERAL := 0.3          # смещение бьющего вбок под опорную ногу (правая → левее), м
