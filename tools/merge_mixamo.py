...
# переставляет оси при экспорте bone-local кривых), и старые предположения по этому
# поводу (в т.ч. более ранняя версия этого комментария про "безопасную дугу ~0.71м
# на индексе 1") оказались неверны на текущем tackle.fbx — перепроверено эмпирически
# заново семплированием сырых pose.bones["Hips"].location keyframe-значений ДО
# заморозки (см. tests/check_tackle_in_place.gd для проверки итога в glb):
# local_index=0 range=26.5, local_index=1 range=71.3, local_index=2 range=442.7
# (сырые Blender-единицы). При действовавшей тогда заморозке (0, 2) экспортированный
# Position3D-трек Hips в glb давал X=0.006 Y=0.0 Z=71.29 — т.е. index 1 остался живым
# и НАПРЯМУЮ (без масштабирования) утёк в экспортированную ось Z, воспроизводя тот же
# баг "разъезжания" root motion, который заморозка должна была устранить. Морозим все
# три оси для tackle, как и для roll_left/roll_right.
